...
//
// This file is a standalone port of MiniMetersVisualizer.tsx (a renderer for
// Konsl's spicetify-visualizer). All the drawing is plain canvas; only the shell
// (button + full-screen overlay + render loop) is new.
// =============================================================================
(function MiniMeters() {
	const S_API = window.Spicetify;
	if (!(S_API && (S_API.Playbar?.Button || S_API.Topbar?.Button) && S_API.Player && document.body)) {
		setTimeout(MiniMeters, 300);
		return;
	}
	...
	function open() {
		...
		bar.innerHTML = "<span>MiniMeters &mdash; real-output metering</span>";
	...
	if (S_API.Playbar && S_API.Playbar.Button) {
		new S_API.Playbar.Button("MiniMeters", ICON, toggle, false, false);
	} else {
		new S_API.Topbar.Button("MiniMeters", ICON, toggle);
	}
